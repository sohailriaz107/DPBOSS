...
if(is_numeric($game_result_tmp))
{
	...
	}else{$free_game_pass_text_to_print ='';}
}
	...
	if (strpos(date('h:i A', strtotime($timeOpen)), 'PM') && strpos(date('h:i A', strtotime($timeClose)), 'AM')) {
		...
		if($time1 >= $startTime && $time1 <= $endTime){
			...
			<!-- <div> -->
				...
				<a href="https://dpboss.boston/" rel="nofollow noopener" target="_top" style=" border:1px solid #e6e6e6;background:#522f92;color:#fff;padding:5px 7px;border-radius:8px;font-size:12px;margin:2px 0 -1px;display:inline-block;transition:all .3s;">Refresh</a>
				<?php
				...
				if( $free_game_pass_text_to_print !=''){ echo '<p>'.preg_replace('/(<br>)+$/', '', $free_game_pass_text_to_print).'</p>';}
				?>
			<!-- </div> -->
...
elseif ($frequency==6) {
	...
	if($day == "mon"){
	    ...
	    <!-- <div> -->
			...
			<hr><a href="<?php echo $app_link_url;?>" target="_blank" style="color: yellow;background: red;padding: 5px;display: block;width: 250px;margin: auto;border: 1px solid black;"><?php echo $app_link_text;?></a><hr>	
		...
		<!-- </div> -->
		<?php }
		elseif ($time1 >= $starttimeC && $time1 <= $endtimeC) {
		if ($updateDate==date('d-m-Y')) { ?>
			<!-- <div> -->
				<span class="h8"><?php echo $gameName ?></span>
				<span class="h9"><?php echo $gameResult ?></span>
